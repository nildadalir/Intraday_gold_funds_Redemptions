...
Filename: `intra-day-gold-redemptions-YYYY-MM-DD-HH-MM.html` (Gregorian, Tehran; hyphens in the time so Windows can store the file). Display / email subject: `gold_redemptions YYYY-MM-DD HH:MM`. The HTML page title inside the file is unchanged. HTML older than `report.retention_days` (10) is deleted from `output/` and `output_error/` on each run.
